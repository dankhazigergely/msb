...
interface UseSurebet8WayProps {
    ...
    stake7: number;
    stake8: number;
    fixedField: StakeField8Way;
}

interface UseSurebet8WayResult {
    stake1: number;
    stake2: number;
    stake3: number;
    stake4: number;
...
export function useSurebet8Way({
    odds1, odds2, odds3, odds4, odds5, odds6, odds7, odds8,
    totalStake,
    stake1, stake2, stake3, stake4, stake5, stake6, stake7, stake8,
    fixedField
}: UseSurebet8WayProps): UseSurebet8WayResult {
    const [result, setResult] = useState<UseSurebet8WayResult>({
    ...
    useEffect(() => {
        ...
        if (isNaN(n1) || isNaN(n2) || isNaN(n3) || isNaN(n4) || isNaN(n5) || isNaN(n6) || isNaN(n7) || isNaN(n8)) {
            setResult(r => ({ ...r, profit: 0, profitPercentage: 0 }));
            return;
        }
        // Számítjuk a szorzatokat (minden odds szorzata kivéve egy)
        const p1 = n2 * n3 * n4 * n5 * n6 * n7 * n8;
        const p2 = n1 * n3 * n4 * n5 * n6 * n7 * n8;
        const p3 = n1 * n2 * n4 * n5 * n6 * n7 * n8;
        const p4 = n1 * n2 * n3 * n5 * n6 * n7 * n8;
        const p5 = n1 * n2 * n3 * n4 * n6 * n7 * n8;
        const p6 = n1 * n2 * n3 * n4 * n5 * n7 * n8;
        const p7 = n1 * n2 * n3 * n4 * n5 * n6 * n8;
        const p8 = n1 * n2 * n3 * n4 * n5 * n6 * n7;
        const denom = p1 + p2 + p3 + p4 + p5 + p6 + p7 + p8;
        const productAll = n1 * n2 * n3 * n4 * n5 * n6 * n7 * n8;
        ...
        if (fixedField === "total") {
            ...
            setResult({
                stake1: s1, stake2: s2, stake3: s3, stake4: s4, stake5: s5, stake6: s6, stake7: s7, stake8: s8,
            ...
            const total = sFixed + s2 + s3 + s4 + s5 + s6 + s7 + s8;
            const profit = sFixed * n1 - total;
            setResult({ stake1: sFixed, stake2: s2, stake3: s3, stake4: s4, stake5: s5, stake6: s6, stake7: s7, stake8: s8, totalStake: total.toFixed(0), profit, profitPercentage });
            ...
            const total = s1 + sFixed + s3 + s4 + s5 + s6 + s7 + s8;
            const profit = s1 * n1 - total;
            setResult({ stake1: s1, stake2: sFixed, stake3: s3, stake4: s4, stake5: s5, stake6: s6, stake7: s7, stake8: s8, totalStake: total.toFixed(0), profit, profitPercentage });
            ...
            const total = s1 + s2 + sFixed + s4 + s5 + s6 + s7 + s8;
            const profit = s1 * n1 - total;
            setResult({ stake1: s1, stake2: s2, stake3: sFixed, stake4: s4, stake5: s5, stake6: s6, stake7: s7, stake8: s8, totalStake: total.toFixed(0), profit, profitPercentage });
            ...
            const total = s1 + s2 + s3 + sFixed + s5 + s6 + s7 + s8;
            const profit = s1 * n1 - total;
            setResult({ stake1: s1, stake2: s2, stake3: s3, stake4: sFixed, stake5: s5, stake6: s6, stake7: s7, stake8: s8, totalStake: total.toFixed(0), profit, profitPercentage });
            ...
            const total = s1 + s2 + s3 + s4 + sFixed + s6 + s7 + s8;
            const profit = s1 * n1 - total;
            setResult({ stake1: s1, stake2: s2, stake3: s3, stake4: s4, stake5: sFixed, stake6: s6, stake7: s7, stake8: s8, totalStake: total.toFixed(0), profit, profitPercentage });
            ...
            const total = s1 + s2 + s3 + s4 + s5 + sFixed + s7 + s8;
            const profit = s1 * n1 - total;
            setResult({ stake1: s1, stake2: s2, stake3: s3, stake4: s4, stake5: s5, stake6: sFixed, stake7: s7, stake8: s8, totalStake: total.toFixed(0), profit, profitPercentage });
            ...
            const total = s1 + s2 + s3 + s4 + s5 + s6 + sFixed + s8;
            const profit = s1 * n1 - total;
            setResult({ stake1: s1, stake2: s2, stake3: s3, stake4: s4, stake5: s5, stake6: s6, stake7: sFixed, stake8: s8, totalStake: total.toFixed(0), profit, profitPercentage });
            ...
            const total = s1 + s2 + s3 + s4 + s5 + s6 + s7 + sFixed;
            const profit = s1 * n1 - total;
            setResult({ stake1: s1, stake2: s2, stake3: s3, stake4: s4, stake5: s5, stake6: s6, stake7: s7, stake8: sFixed, totalStake: total.toFixed(0), profit, profitPercentage });
        }
    }, [odds1, odds2, odds3, odds4, odds5, odds6, odds7, odds8, totalStake, stake1, stake2, stake3, stake4, stake5, stake6, stake7, stake8, fixedField]);
