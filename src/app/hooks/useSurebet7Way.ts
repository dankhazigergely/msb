...
interface UseSurebet7WayProps {
    ...
    stake6: number;
    stake7: number;
    fixedField: StakeField7Way;
}

interface UseSurebet7WayResult {
    stake1: number;
    stake2: number;
    stake3: number;
    stake4: number;
...
export function useSurebet7Way({
    odds1, odds2, odds3, odds4, odds5, odds6, odds7,
    totalStake,
    stake1, stake2, stake3, stake4, stake5, stake6, stake7,
    fixedField
}: UseSurebet7WayProps): UseSurebet7WayResult {
    const [result, setResult] = useState<UseSurebet7WayResult>({
    ...
    useEffect(() => {
        ...
        const p1 = n2 * n3 * n4 * n5 * n6 * n7;
        const p2 = n1 * n3 * n4 * n5 * n6 * n7;
        const p3 = n1 * n2 * n4 * n5 * n6 * n7;
        const p4 = n1 * n2 * n3 * n5 * n6 * n7;
        const p5 = n1 * n2 * n3 * n4 * n6 * n7;
        const p6 = n1 * n2 * n3 * n4 * n5 * n7;
        const p7 = n1 * n2 * n3 * n4 * n5 * n6;
        const denom = p1 + p2 + p3 + p4 + p5 + p6 + p7;
        const productAll = n1 * n2 * n3 * n4 * n5 * n6 * n7;
        ...
        if (fixedField === "total") {
            ...
            setResult({
                stake1: s1, stake2: s2, stake3: s3, stake4: s4, stake5: s5, stake6: s6, stake7: s7,
            ...
            const total = sFixed + s2 + s3 + s4 + s5 + s6 + s7;
            const profit = sFixed * n1 - total;
            setResult({ stake1: sFixed, stake2: s2, stake3: s3, stake4: s4, stake5: s5, stake6: s6, stake7: s7, totalStake: total.toFixed(0), profit, profitPercentage });
            ...
            const total = s1 + sFixed + s3 + s4 + s5 + s6 + s7;
            const profit = s1 * n1 - total;
            setResult({ stake1: s1, stake2: sFixed, stake3: s3, stake4: s4, stake5: s5, stake6: s6, stake7: s7, totalStake: total.toFixed(0), profit, profitPercentage });
            ...
            const total = s1 + s2 + sFixed + s4 + s5 + s6 + s7;
            const profit = s1 * n1 - total;
            setResult({ stake1: s1, stake2: s2, stake3: sFixed, stake4: s4, stake5: s5, stake6: s6, stake7: s7, totalStake: total.toFixed(0), profit, profitPercentage });
            ...
            const total = s1 + s2 + s3 + sFixed + s5 + s6 + s7;
            const profit = s1 * n1 - total;
            setResult({ stake1: s1, stake2: s2, stake3: s3, stake4: sFixed, stake5: s5, stake6: s6, stake7: s7, totalStake: total.toFixed(0), profit, profitPercentage });
            ...
            const total = s1 + s2 + s3 + s4 + sFixed + s6 + s7;
            const profit = s1 * n1 - total;
            setResult({ stake1: s1, stake2: s2, stake3: s3, stake4: s4, stake5: sFixed, stake6: s6, stake7: s7, totalStake: total.toFixed(0), profit, profitPercentage });
            ...
            const total = s1 + s2 + s3 + s4 + s5 + sFixed + s7;
            const profit = s1 * n1 - total;
            setResult({ stake1: s1, stake2: s2, stake3: s3, stake4: s4, stake5: s5, stake6: sFixed, stake7: s7, totalStake: total.toFixed(0), profit, profitPercentage });
            ...
            const total = s1 + s2 + s3 + s4 + s5 + s6 + sFixed;
            const profit = s1 * n1 - total;
            setResult({ stake1: s1, stake2: s2, stake3: s3, stake4: s4, stake5: s5, stake6: s6, stake7: sFixed, totalStake: total.toFixed(0), profit, profitPercentage });
        }
    }, [odds1, odds2, odds3, odds4, odds5, odds6, odds7, totalStake, stake1, stake2, stake3, stake4, stake5, stake6, stake7, fixedField]);
